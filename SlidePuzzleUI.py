...
# Código para resoluçăo do problema dos blocos deslizantes através do algoritmo de busca A*, usando a heurística de distância de Manhattan


# Interface gráfica adaptada da soluçăo proposta por Al Sweigart:
...
def main():
    ...
    pygame.display.set_caption('Jogo dos 15 - Trabalho de Inteligęncia Artificial')
            ...
            if event.type == MOUSEBUTTONUP:
                ...
                if (spotx, spoty) == (None, None):
                    if RESET_RECT.collidepoint(event.pos):
                        ...
                        msg = 'Buscando soluçăo...' # contains the message to show in the upper left corner.
                        drawBoard(mainBoard, msg)
                        solve(mainBoard, allMoves)
                        msg = 'Soluçăo encontrada. Resolvendo...' # contains the message to show in the upper left corner.
                        drawBoard(mainBoard, msg)
                else:
...
def getLoadedBoard():
    # Carrega um tabuleiro conforme preenchido na constante LOADEDBOARD, instanciada no ínicio desse aquivo
...
def drawBoard(board, message):
    ...
    #Contador de steps computados realizados (multiplos passos em uma mesma direçăo contam apenas como 1)
...
def solveAnimation(board, solutionMoves):
    # Originalmente, o código do UI realizava a inversăo de todos os movimentos feitos desde que o tabuleiro foi inicializado e embaralhado
    # Essa funçăo foi adaptada para realizar os passos calculados pelo algoritmo de busca
    revAllMoves = solutionMoves[:] # gets a copy of the list  
    for move in revAllMoves:
        slideAnimation(board, move, 'Resolvendo através de busca...', animationSpeed=int(ANIMSPEED))
        makeMove(board, move)  
        allMoves.append(move)


    # Converte o estilo de lista utilizado pela UI para o estilo de lista em uma dimensăo usado para o algoritmo A*
def uiBoardToArray(mainboard):
    ...
    # Funçăo responsável por preparar, solucionar via A* e animar a resoluçăo do tabuleiro
def solve(board, allMoves):
    array = uiBoardToArray(board)
    solutionMoves = solveBoardFun(array, BOARDWIDTH) #chama a funçăo no arquivo solveBoard.py 
    #print(solutionMoves)
